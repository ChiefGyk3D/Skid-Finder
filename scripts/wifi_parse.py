...
# tshark -T fields -e ... in exactly this order. Keep in step with
# scripts/wifi-capture.sh and scripts/wifi-live-watch.sh.
FIELDS = [
    "frame.time_epoch",
...
def parse_line(line: str) -> Optional[WifiFrame]:
    ...
    frame = WifiFrame()
    try:
        frame.timestamp = float(parts[0]) if parts[0].strip() else None
    except ValueError:
        frame.timestamp = None
    frame.subtype = parse_subtype(parts[1])
    frame.sa = parts[2].strip().upper() if MAC_RE.match(parts[2].strip()) else ""
    frame.da = parts[3].strip().upper() if MAC_RE.match(parts[3].strip()) else ""
    frame.bssid = parts[4].strip().upper() if MAC_RE.match(parts[4].strip()) else ""
    frame.ssid = parse_ssid(parts[5])
